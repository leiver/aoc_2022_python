...
hand_mapping = {"X": "A", "Y": "B", "Z": "C"}

hand_score_mapping = {"A": 1, "B": 2, "C": 3}
...
def determine_score_for_round(round):
    ...
    game_result = game_result_order[
        (3 + hand_score_mapping[first_player] - hand_score_mapping[second_player]) % 3
    ]
...
def map_round_by_part_2_strategy(round):
    ...
    second_player = possible_hands[
        (3 + hand_score_mapping[first_player] - 1 + strategy_mapping[strategy]) % 3
    ]

    return f"{first_player} {second_player}"


rounds = get_input(2).rstrip().split("\n")
